...
doc = """
Tanaka 2010 version of the Multiple price list task as proposed by Holt/Laury (2002), American Economic Review 92(5).
"""
    ...
    def creating_session(self):
        if self.round_number == 1:
            ...
            for p in self.get_players():
                ...
                # Creando lista de pagos crecientes TANAKA 1
                payoffs = [
                    "{0:.2f}".format(k / n * 100)
                    for k in indices
                ]
...
class Player(BasePlayer):
    ...
    def set_payoffs(self):
        ...
        # RANDOM DRAW NUEVO: PARA PROB EXACTAS (11 no se generara, sino vals del 1 al 10)
        ...
        # ------------------------------------------------------------------------------------------------------------
        self.option_to_pay = getattr(self, self.choice_to_pay)     #getattr(self, self.choice_to_pay) = player.choice_(index to pay)

        # set player's payoff
        # ------------------------------------------------------------------------------------------------------------
        if self.option_to_pay == 'A':
            # Segun este metodo, aqui se det la prob de pago de las loterias. Si quiero que se pague en A con 30% de
            # prob, debo jugar con el random_draw
